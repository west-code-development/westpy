...
class eBSEResultTestCase(unittest.TestCase):
    ...
    def test_class_variables(self):
        ...
        """
        self.assertEqual(
            self.ebseresult.filename,
            str(Path("./data").resolve() / "wfreq_spinpol.json"),
        )
        ...
        self.assertListEqual(
            self.ebseresult.jwstring.tolist(), self.ebse_ref["jwstring"]
        )
    ...
    def test_solution(self):
        ...
        np.testing.assert_almost_equal(
            solution["hamiltonian"], np.array(self.solution_ref["hamiltonian"])
        )
        np.testing.assert_almost_equal(
            solution["evs"], np.array(self.solution_ref["evs"])
        )
        np.testing.assert_almost_equal(
            solution["evs_au"], np.array(self.solution_ref["evs_au"])
        )
        np.testing.assert_almost_equal(
            np.abs(solution["evcs"]), np.abs(np.array(self.solution_ref["evcs"]))
        )
        np.testing.assert_almost_equal(
            solution["rdm1s"], np.array(self.solution_ref["rdm1s"])
        )
        np.testing.assert_almost_equal(
            solution["mults"], np.array(self.solution_ref["mults"])
        )
