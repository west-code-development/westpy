...
class QDETResultTestCase(unittest.TestCase):
    ...
    def test_class_variables(self):
        ...
        """
        self.assertEqual(
            self.qdetresult.filename, str(Path("./data").resolve() / "wfreq.json")
        )
        self.assertEqual(self.qdetresult.point_group, None)
        self.assertEqual(self.qdetresult.nspin, 1)
        self.assertListEqual(
            self.qdetresult.basis.tolist(), [87, 122, 123, 126, 127, 128]
        )
        self.assertEqual(self.qdetresult.npair, 21)
        self.assertEqual(
            self.qdetresult.occupation.tolist(), [[2.0, 2.0, 2.0, 2.0, 1.0, 1.0]]
        )
