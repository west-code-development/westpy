...
class QDETResultTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.qdetresults = QDETResults(path=self.path)
    ...
    def test_class_variables(self):
        ...
        """
        self.assertEqual(self.qdetresults.path, str(Path('./data')))
        self.assertEqual(self.qdetresults.eps_infty, None)
        self.assertEqual(self.qdetresults.point_group, None)
        self.assertEqual(self.qdetresults.nspin, 1)
        self.assertEqual(self.qdetresults.npdep, 506)
        self.assertEqual(self.qdetresults.omega, 4142.0863696677025)
        self.assertEqual(self.qdetresults.nproj, 6)
        self.assertEqual(self.qdetresults.ks_projectors, [252, 253, 254, 255, 256, 257])
        self.assertEqual(self.qdetresults.basis, [0, 1, 2, 3, 4, 5])
        self.assertEqual(self.qdetresults.npair, 21)
        self.assertEqual(self.qdetresults.div, 0.3042077)
        self.assertEqual(self.qdetresults.nel, 4)
    ...
    def test_quasiparticle_properties(self):
        ...
        sigmax_n_f_ref = np.array([[[-9.35359783e-01,  3.96457100e-07,  1.73876912e-07,
        ...
        np.testing.assert_almost_equal(np.diagonal(self.qdetresults.qp_energy_n[0]),
                qp_energies_ref)
        np.testing.assert_almost_equal(self.qdetresults.occ, occ_ref)
        
        np.testing.assert_almost_equal(self.qdetresults.sigmax_n_a, sigmax_n_a_ref)
        np.testing.assert_almost_equal(self.qdetresults.sigmax_n_e, sigmax_n_e_ref)
        np.testing.assert_almost_equal(self.qdetresults.sigmax_n_f, sigmax_n_f_ref)
