import unittest
import json
from westpy.qdet import PointGroup, PointGroupOperation, PointGroupRotation, PointGroupReflection
...
class SymmetryTestCase(unittest.TestCase):
    def setUp(self):
        ...
        filenames = [str(Path('./data').resolve() / 'wfcK000001B000126.cube'),
    ...
    def test_point_group_rep(self):
        ...
        point_group_rep = self.point_group.compute_rep_on_orbitals(self.orbitals,
                orthogonalize=True)[0]

        for key in point_group_rep.rep_matrices.keys():
            np.testing.assert_almost_equal(point_group_rep.rep_matrices[key],
