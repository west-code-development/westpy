...
class bfgs_iter:
    ...
    def __init__(
        ...
        if l_restart:
            self.start_iter = self._read_restart()
        else:
            self.start_iter = 0
        self._read_prefix()
        self._read_pos_unit()
    ...
    def _read_pos_unit(self):
        ...
        if not pos_unit in ["angstrom", "crystal", "bohr"]:
            self._log(
                f"{pos_unit} unit for ATOMIC_POSITIONS is currently not supported"
            )
            exit()
    ...
    def _run_calc(self):
        ...
        try:
            ...
            self._log(f"pw.x failed: {work_dir}")
            exit()

        # load number of atoms, lattice parameters, atomic species, and atomic positions
        self._read_constants_positions_pwxml()
        # load ground state forces and energy
        self._read_ground_state_forces_energy_pwxml()

        # run nscf
        if self.run_nscf:
            self._log("Running nscf pw.x ...")
            ...
            try:
                ...
                self._log(f"nscf pw.x failed: {work_dir}")
                exit()
        ...
        try:
            ...
            self._log(f"wbse.x failed: {work_dir}")
            exit()
    ...
    def _initialize(self):
        ...
        if self.scf_iter == 1:
            ...
            self.trust_radius_old = self._scnorm(self.step_old)
    ...
    def solve(self):
        ...
        """
        self._log("BFGS Geometry Relaxation")
        self._log("")
        ...
        for scf_iter in range(self.start_iter, self.maxiter):
            ...
            self._log("")
            self._log(f"number of scf cycles    = {self.scf_iter:3d}")
            self._log(f"number of bfgs steps    = {self.bfgs_iter:3d}")
            if self.scf_iter > 1:
                self._log(f"energy old              = {self.energy_p:18.10f} Ry")
            self._log(f"energy new              = {self.energy:18.10f} Ry")
            self.grad_t = np.linalg.norm(np.dot(self.hinv_block.T, self.grad))
            self._log(f"total force             = {self.grad_t:18.10f} Ry/Bohr")
            ...
            if self.conv_bfgs:
                ...
                self._save_final_geo()
                break

            self._compute_new_position()
            self._write_new_pw_input()
            self._log("")
    ...
    def _save_final_geo(self):
        ...
        self._log("")
        self._log(f"Final geometry saved to {save_dir}")
    ...
    def _compute_new_position(self):
        ...
        self._check_energy_condition()
        if (not self.e_wolfe) and (self.scf_iter > 1):
            # the previous step is rejected, line search goes on
            self.step_accepted = False
            #
            self._log("CASE: energy_new > energy_old")

            if abs(self._scnorm(self.step_old) - 1.0) > 1.0e-10:
                self._log("step_old is NOT normalized")
                exit()
            ...
            if self.bfgs_iter == 1:
                ...
                self._log("CASE: energy_new < energy_old")
                self._check_wolfe_conditions()
                self._update_inverse_hessian()
            ...
            if self.bfgs_iter == 1:
                ...
                self._compute_trust_radius()

            self._log(f"new trust radius        = {self.trust_radius:18.10f} Bohr")

        if self.nr_step_length < 1.0e-16:
            self._log("NR step-length unreasonably short")
            exit()
    ...
    def _write_new_pw_input(self):
        ...
        if self.l_copy_save_dir:
            # copy .save directory from current to next step
            ...
            self._log("")
            self._log(f"Copied {self.pw_prefix}.save folder into {next_dir}")
        shutil.rmtree(work_dir)

    def _reset_bfgs(self):
        """
        inv_hess is re-initialized to initial guess
        """
        self.inv_hess = np.linalg.inv(self.metric)

    def _update_inverse_hessian(self):
    ...
    def _compute_trust_radius(self):
        ...
        if self.trust_radius < self.trust_radius_min:
            ...
            self._log("small trust_radius: resetting bfgs history")
            self._reset_bfgs()
            self.step = -np.dot(self.inv_hess, self.grad)
            self.nr_step_length = self._scnorm(self.step)
            self.step = self.step / self.nr_step_length
    ...
    def _terminate_bfgs(self):
        if self.conv_bfgs:
            ...
            self._log("")
            self._log("End of BFGS Geometry Optimization")
            self._log("")
            self._log(f"Final energy = {self.energy:18.10f} Ry")
        else:
            self._log("The maximum number of steps has been reached")

    def _read_constants_positions_pwxml(self):
        """
    ...
    def _read_ground_state_forces_energy_pwxml(self):
        """
    ...
    def _read_excited_state_forces_energy(self):
        """
