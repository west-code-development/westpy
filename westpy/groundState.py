    ...
    def generateInputPW(self, fname="pw.in"):
        ...
        if self.geom.pseudoFormat in ["upf"]:
            ...
            with open(fname, "w") as file:
                ...
                file.write("ibrav             = 0\n")
                file.write(
                    "nat               = " + str(self.geom.getNumberOfAtoms()) + "\n"
                )
                file.write(
                    "ntyp              = " + str(self.geom.getNumberOfSpecies()) + "\n"
                )
                file.write("ecutwfc           = " + str(self.ecut) + "\n")
                file.write(
                    "nbnd              = "
                    + str(self.geom.getNumberOfElectrons() + self.nempty)
                    + "\n"
                )
                ...
                if "nspin" in self.spin.keys():
                    if self.spin["nspin"] == 2:
                        ...
                        file.write(
                            "tot_magnetization = "
                            + str(self.spin["tot_magnetization"])
                            + "\n"
                        )
                    if self.spin["nspin"] == 4:
                        ...
                        file.write(
                            "lspinorb          = "
                            + logical2str(self.spin["lspinorb"])
                            + "\n"
                        )
                if self.isolated:
                ...
                for atom in self.geom.atoms:
                    if atom.symbol not in sp:
                        ...
                        file.write(
                            atom.symbol
                            + " "
                            + str(self.geom.species[atom.symbol]["mass"])
                            + " "
                            + self.geom.species[atom.symbol]["fname"]
                            + "\n"
                        )
                #
            ...
            with open(fname, "w") as file:
                ...
                for atom in self.geom.atoms:
                    if atom.symbol not in sp:
                        ...
                        file.write(
                            ...
                            + " "
                            + self.geom.species[atom.symbol]["url"]
                            + "\n"
                        )
                # atom
                i = 0
                ...
                if "nspin" in self.spin.keys():
                    if self.spin["nspin"] == 2:
                        ...
                        delta_spin = (
                            self.spin["tot_magnetization"]
                            - (self.geom.getNumberOfElectrons() % 2)
                        ) / 2
                        file.write(
                            "set delta_spin"
                            + str(self.spin["tot_magnetization"])
