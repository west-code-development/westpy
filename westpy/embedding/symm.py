...
class PointGroupOperation:
  def __init__(self, T: np.ndarray, origin: Union[Sequence[float], np.ndarray] = None):
...
class PointGroupReflection(PointGroupOperation):
  def __init__(self,
               ...
               origin: Union[Sequence[float], np.ndarray] = (0., 0., 0.)):
...
class PointGroupInversion(PointGroupOperation):
  def __init__(self, origin: Union[Sequence[float], np.ndarray] = (0., 0., 0.)):
    """ Inversion operation.

    Args:
        origin: origin.
    """
    super(PointGroupInversion, self).__init__(T=block_diag(-1 * np.eye(3), 1), origin=origin)


class PointGroup:
  def __init__(self, name: str, operations: Sequence[PointGroupOperation], ctable: Dict[str, Sequence]):
  ...
  def compute_rep_on_orbitals(self,
    ...
    """
    rep = PointGroupRep(point_group=self, orbitals=orbitals, orthogonalize=orthogonalize)
...
class PointGroupRep:
  def __init__(self,
    ...
    self.rep_matrices = {R: np.zeros([self.norb, self.norb]) for R in self.point_group.operations}
    ...
    if all([np.all(np.isclose(D @ D.T, np.eye(self.norb)))
