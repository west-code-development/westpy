...
class Wavefunction:
    ...
    def read_data(self, volume, ordering="xyz", norm="qe"):
            ...
            for i in range(self.npoints[0]):
                for j in range(self.npoints[1]):
                    for k in range(self.npoints[2]):
                        self.data[i, j, k] = np.sqrt(np.abs(data_[index])) * np.sign(
                            data_[index]
                        )
                        index += 1
        elif ordering == "zyx":
        ...
        if norm == "qe":
            self.data[:, :, :] = self.data[:, :, :] * np.sqrt(
                volume / (self.npoints[0] * self.npoints[1] * self.npoints[2])
            )
        elif norm == "west":
            self.data[:, :, :] = self.data[:, :, :] * np.sqrt(
                1.0 / (self.npoints[0] * self.npoints[1] * self.npoints[2])
            )
    ...
    def integrate(self, box=None):
        if box == None:
            ...
            grid = [
                np.linspace(0, 1, num=self.npoints[0], endpoint=False),
        ...
        for ix in range(self.data.shape[0]):
            for iy in range(self.data.shape[1]):
                for iz in range(self.data.shape[2]):
                    point = [
                        float(ix) / float(self.npoints[0]),
                        float(iy) / float(self.npoints[1]),
                        float(iz) / float(self.npoints[2]),
                    ]
