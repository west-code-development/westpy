...
class WstatOutput:
  ...
  def make_renormalized_fxc(self, alpha=0.25, fxclim=-1,
                            renormalization='cut', fxc_treatment='re-diagonalize',
                            export=None):
    ...
    if fxc_treatment == "re-diagonalize":
      ...
      for i in range(self.npdep):
        if renormalization == "erf":
          fxcr[i, i] = (erfc(alpha * i / self.nel) * self.fxc[i, i]
    ...
    if export is None:
      ...
      fxcfile[find_index("<fxc", fxcfile) + 1: find_index("</fxc>", fxcfile)] = [
        "{:.15e}\n".format(fxcr[i, j]) for i, j in np.ndindex(self.npdep, self.npdep)
      ]
  ...
  def make_fxc_file(fxc, fname):
    ...
    open(fname, "w").write(
      _fxc_template.format(npdep=npdep, fxcdim=fxc.size,
...
class WfreqOutput:
  ...
  def __init__(self, outpath=None, savepath="."):
    if outpath is not None:
      ...
      self.qp1 = parse_many_values(2, int, ofile[find_index("qp_bandrange(1)", ofile)])[1]
      self.qp2 = parse_many_values(2, int, ofile[find_index("qp_bandrange(2)", ofile)])[1]
  ...
  def plot_spectral_analysis(self, ikpt, ibnd, in_place=True, title="",
                             plot_linear=False, plot_sigma=False,
                             plot_width=2, find_maximum=False):
    ...
    sigmac_eks_re = self.js["output"]["Q"]["K{:06}".format(ikpt)]["sigmac_eks"]['re'][ibnd]
    ...
    sigmac_re = np.array(self.js["output"]["P"]["K{:06}".format(ikpt)]["B{:06}".format(ibnd + 1)]["sigmac"]["re"])
    sigmac_im = np.array(self.js["output"]["P"]["K{:06}".format(ikpt)]["B{:06}".format(ibnd + 1)]["sigmac"]["im"])
    ...
    A = (1 / np.pi) * np.abs(sigma.imag) / ((omega - eks - sigma.real + vxc) ** 2 + (sigma.imag) ** 2)
    ...
    quantities_info = {
      ...
      "Q": {"x": omega, "y": Q,
            "label": "$Q$", "color": "green"},
      "Qlin": {"x": omega, "y": Qlin,
               "label": "$Q^{\mathrm{lin}}$", "color": "red"},
      "sigmac_re": {"x": omega, "y": sigmac_re,
                    "label": "$Re(\Sigma_c)$", "color": "purple"},
      "sigmac_im": {"x": omega, "y": sigmac_im,
                    "label": "$Im(\Sigma_c)$", "color": "brown"},
    }
    ...
    ax.set_ylim(min(min(y[imin:imax]) for y in ys), max(max(y[imin:imax]) for y in ys))

    auxlines_info = {
      "zero": {"x": ax.get_xlim(), "y": [0, 0], "color": "black"},
      "eqpsec": {"x": [eqpsec, eqpsec], "y": ax.get_ylim(),
                 "label": "$E^{\mathrm{QP}}$", "linestyle": "--", "color": "green"},
      "eqplin": {"x": [eqplin, eqplin], "y": ax.get_ylim(),
                 "label": "$E^{\mathrm{QP-lin}}$", "linestyle": "--", "color": "red"},
      "sigmax": {"x": ax.get_xlim(), "y": [sigmax, sigmax],
                 "label": "$\Sigma_x$", "color": "yellow"},
    }
    ...
    label_order = ["$A$", "$Q$", "$E^{\mathrm{QP}}$",
                   "$Q^{\mathrm{lin}}$", "$E^{\mathrm{QP-lin}}$",
                   "$\Sigma_x$", "$Re(\Sigma_c)$", "$Im(\Sigma_c)$"]
    handles, labels = ax.get_legend_handles_labels()
    hls = sorted(zip(handles, labels),
