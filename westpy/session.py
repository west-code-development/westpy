class Session(object):
    ...
    def run(
        ...
        if executable in self.allowedExecutables:
            ...
            try:
                output = self.__runExecutable(
                    executable, inputFile, downloadUrl, number_of_cores
                )
    ...
    def __runExecutable(self, executable, input_file, download_urls, number_of_cores):
        ...
        body = {
            "urls": download_urls,
            "file": file_content,
            "cmd_timeout": str(self.maxWaitTime),
            "script_type": str(executable),
            "no_of_cores": str(number_of_cores),
        }
        ...
        try:
            response = requests.post(
                self.restAPIrun, data=jsondataasbytes, headers=headers, timeout=None
            )
